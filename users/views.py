...
from users.models import itemfoundfull, itemlostfull, Profile
...
def register(request):
    if request.method == 'POST':
        ...
        if formregi.is_valid():
            user = formregi.save()
            # Профиль создается через сигнал, но на всякий случай:
            Profile.objects.get_or_create(user=user, defaults={'image': 'default.jpg'})
